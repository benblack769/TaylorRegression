...
protected:
	...
	void FindBinaryMean(){
		vector <double> Means;
		Means.resize(Size);
		Means[0] = (Nums[0] + Nums[1]) / 2;
		int UpperBound = Size - 2;
		for(int n = 1; n < UpperBound; n++){
			Means[n] = (Nums[n - 1] + Nums[n] + Nums[n + 1]) / 3;
		}
		Means[Size - 1] = (Nums[Size - 2] + Nums[Size - 1]) / 2;
		Nums = Means;
	}
